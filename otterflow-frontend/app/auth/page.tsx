  ...
  export default function AuthPage() {
    ...
    const handleForgotPassword = async (e: React.FormEvent) => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          toast.error(data.detail || 'User with this email does not exist. Please use a valid email or signup.')
        }
      } catch (error) {
    ...
    const handleLinkedInLogin = async () => {
      try {
        const backendUrl = process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:8000'
        const response = await fetch(`${backendUrl}/auth/login/linkedin`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
          },
            ...
            <div className="p-8">
                ...
                <TabsContent value="login">
                  {step === 'form' && (
                    <form onSubmit={handleLogin} className="space-y-4">
                      ...
                      <Button type="submit" className="w-full bg-teal-600 hover:bg-teal-700">Log in</Button>
                    </form>
                  )}
                  {step === 'forgot_password' && (
                    <form onSubmit={handleForgotPassword} className="space-y-4">
                      ...
                      <Button type="submit" className="w-full bg-teal-600 hover:bg-teal-700">Send Reset OTP</Button>
                    </form>
                  )}
                </TabsContent>
                <TabsContent value="signup">
                  {step === 'form' && (
                    <form onSubmit={handleSignup} className="space-y-4">
                      ...
                      <Button type="submit" className="w-full bg-teal-600 hover:bg-teal-700">Sign up</Button>
                    </form>
                  )}
                  {step === 'verify_email' && (
                    <form onSubmit={handleVerifyEmail} className="space-y-4">
                      ...
                      <Button type="submit" className="w-full bg-teal-600 hover:bg-teal-700">Verify Email</Button>
                    </form>
                  )}
                </TabsContent>
              </Tabs>
              ...
              {step === 'reset_password' && (
                <form onSubmit={handleResetPassword} className="space-y-4 mt-4">
                  ...
                  <Button type="submit" className="w-full bg-teal-600 hover:bg-teal-700">Reset Password</Button>
                </form>
              )}
                ...
                <>
                  <div className="mt-4">
                    ...
                    <div className="mt-4 grid grid-cols-2 justify-center gap-4">
                      ...
                      <Button
                        variant="outline"
                        className="border-teal-600 justify-center text-teal-300 hover:bg-teal-800/50 flex items-center justify-center w-full"
                  ...
                  <div className="mt-4 text-center text-sm">
                    {isSignUp ? (
                      <Link href="#" className="text-teal-400 hover:underline" onClick={() => {
                        setIsSignUp(false)
                        setStep('form')
                        setFormData({
                          email: '',
